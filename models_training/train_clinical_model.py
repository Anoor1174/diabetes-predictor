...
# Low threshold on purpose — this is a screening tool, so false negatives
# (missed diabetics) are worse than false positives.
THRESHOLD = 0.15
RANDOM_STATE = 42
N_SPLITS = 5
...
def load_data():
    ...
    # Ethnicity codes are nominal, not ordinal, so one-hot rather than leave as int.
...
def cv_recall_at_threshold(estimator_factory, X, y, threshold=THRESHOLD):
    """5-fold stratified CV recall/precision/AUC at a given threshold.

    Scaling and SMOTE are refit inside each fold to avoid leakage.
    """
    skf = StratifiedKFold(
...
def fit_final_model(estimator_factory, X, y):
    """Fit the selected model on a fixed 80/20 split and save the held-out test set."""
    ...
    # Held out untouched so bias_evaluation.py can score fairly on unseen data.
...
def main():
    ...
    # Saved separately so the report can cite CV numbers without rerunning training.
    pd.DataFrame(scores).T.to_csv(
        os.path.join(BASE_DIR, "model_selection_summary.csv")
    )
    print("Saved model, scaler, feature list, test set, and summary.")
