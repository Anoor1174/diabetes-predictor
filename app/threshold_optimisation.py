...
# Match the training pipeline exactly: Sex mapped to 0/1, Ethnicity one-hot encoded.
df["Sex"] = df["Sex"].map({1: 0, 2: 1})

TARGET_COL = "Diabetes"
GROUP_COL = "Ethnicity"  # kept as raw codes for fairness grouping, before one-hot
...
def evaluate_at_threshold(threshold: float) -> dict:
    ...
    fairness_gap_recall = (
        float(np.max(recalls) - np.min(recalls)) if recalls else 0.0
    )
