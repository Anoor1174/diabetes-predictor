...
# Screening threshold prioritises recall over precision (see training scripts).
...
def rule_based_adjustments(data):
    # Manual overrides for extreme values the model alone might undercall.
    rules = []
...
def risk_explanation(category, pathway):
    ...
    suffix = " This screening tool does not replace a clinical diagnosis."
...
def _validate_lifestyle_input(data):
    ...
    try:
        coerced = {
            ...
            "Ethnicity": int(data.get("Ethnicity", 0)),
            "BMI": float(data.get("BMI", 0)),
            # Waist and the fields below are optional. Left blank, the trained
            # imputer fills the population median rather than us assuming 0.
            "WaistCM": (
                float(data["WaistCM"])
